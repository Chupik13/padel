...
export function uploadVideoSegment(matchId: number, cameraSide: number, blob: Blob, contentType: string): Promise<unknown> {
  const formData = new FormData();
  const ext = contentType.includes('mp4') ? 'mp4' : 'webm';
  formData.append('file', blob, `side${cameraSide}.${ext}`);
  return apiFetch(`/api/videos/${matchId}/upload?cameraSide=${cameraSide}`, {
    method: 'POST',
    body: formData,
  });
}
